...
const Firecrackers = () => {
  ...
  const submitForm = (event) => {
    ...
    //post axios call
    axios.post("http://localhost:8181/api/firecracker/firecrackers", datatobesend)
      .then((response) => {

        console.log(response.data);
        localStorage.setItem("firecracker", JSON.stringify(response.data));
        const service1 = localStorage.getItem("firecracker");
  ...
  const updateService = (event) => {
    ...
    let datatobesend = {
      ...
      //axios call for update firecracker
      axios.put("http://localhost:8181/api/firecracker/firecrackers", datatobesend)
      .then((response) => {
  ...
  return (
    <div>
      <h3>Fill Your FireCracker details:</h3>
      <form onSubmit={submitForm}>
        ...
        <div> 
        <pre><Button type='submit'>Register Firecracker</Button>   <Button onClick={updateService}>Update Firecracker</Button> </pre>
        </div>
      </form>
    </div>
  )
