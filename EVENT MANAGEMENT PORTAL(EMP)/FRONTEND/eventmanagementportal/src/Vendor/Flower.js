...
const Flower = () => {
  ...
  const submitForm = (event) => {
    ...
    //post axios call
    axios.post("http://localhost:8181/api/flower/flowers", datatobesend)
      .then((response) => {

        console.log(response.data);
        localStorage.setItem("flower", JSON.stringify(response.data));
        const service1 = localStorage.getItem("flower");
  ...
  const updateService = (event) => {
    ...
    let datatobesend = {
      ...
      //axios call for update flower
      axios.put("http://localhost:8181/api/flower/flowers", datatobesend)
      .then((response) => {
  ...
  return (
    <div>
       <h3>Fill Your Flower details:</h3>
      <form onSubmit={submitForm}>
        ...
        <div> 
        <pre><Button type='submit'>Register Flower</Button>   <Button onClick={updateService}>Update Flower</Button> </pre>
        </div>
      </form>
    </div>
  )
