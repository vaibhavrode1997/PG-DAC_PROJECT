...
const Photography = () => {
  ...
  const submitForm = (event) => {
    ...
    //post axios call
    axios.post("http://localhost:8181/api/photography/photographys", datatobesend)
      .then((response) => {

        console.log(response.data);
        localStorage.setItem("photography", JSON.stringify(response.data));
        const service1 = localStorage.getItem("photography");
  ...
  const updateService = (event) => {
    ...
    let datatobesend = {
      ...
      //axios call for update photography
      axios.put("http://localhost:8181/api/photography/photographys", datatobesend)
      .then((response) => {
  ...
  return (
    <div>
      <h3>Fill Your Photography details:</h3>
      <form onSubmit={submitForm}>
        ...
        <div> 
        <pre><Button type='submit'>Register Photography</Button>   <Button onClick={updateService}>Update Photography</Button> </pre>
        </div>
      </form>
    </div>
  )
