...
const Poojavidhi = () => {
  ...
  const submitForm = (event) => {
    ...
    //post axios call
    axios.post("http://localhost:8181/api/poojavidhi/poojavidhis", datatobesend)
      .then((response) => {

        console.log(response.data);
        localStorage.setItem("poojavidhi", JSON.stringify(response.data));
        const service1 = localStorage.getItem("poojavidhi");
  ...
  const updateService = (event) => {
    ...
    let datatobesend = {
      ...
      //axios call for update poojavidhi
      axios.put("http://localhost:8181/api/poojavidhi/poojavidhis", datatobesend)
      .then((response) => {
  ...
  return (
    <div>
      <h3>Fill Your PoojaVidhi Service details:</h3>
      <form onSubmit={submitForm}>
        ...
        <div> 
        <pre><Button type='submit'>Register Poojavidhi</Button>   <Button onClick={updateService}>Update Poojavidhi</Button> </pre>
        </div>
      </form>
    </div>
  )
